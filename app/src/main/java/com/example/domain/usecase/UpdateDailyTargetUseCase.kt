...
class UpdateDailyTargetUseCase(
    ...
    suspend fun updateGlobalDailyTarget(hours: Int, minutes: Int): Result<Unit> = runCatching {
        val totalMinutes = (hours * 60 + minutes).coerceIn(60, 24 * 60)
        repository.updateDailyRequiredMinutes(totalMinutes)
    }

    suspend fun updateMonthSpecificTarget(year: Int, month: Int, hours: Int, minutes: Int): Result<Unit> = runCatching {
        val totalMinutes = (hours * 60 + minutes).coerceIn(60, 24 * 60)
